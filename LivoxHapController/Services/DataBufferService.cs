...
namespace LivoxHapController.Services
{
    ...
    public class DataBufferService<T>
    {
        ...
        private readonly ObjectPool<DataChunk<T>> _chunkPool =
            //new(() => new DataChunk<T>(LivoxHapQuickStart.POINTS_PER_PKG));
#if NET9_0_OR_GREATER
            new(() => new DataChunk<T>());
#elif NET45_OR_GREATER
        ...
        /// </summary>
#if NET9_0_OR_GREATER
        private volatile int _windowCapacity = LivoxHapQuickStart.POINTS_PER_PKG * LivoxHapQuickStart.PKGS_PER_MILLISEC * 100; // 默认100ms窗口
#elif NET45_OR_GREATER
        private volatile int _windowCapacity = LivoxHapQuickStart.POINTS_PER_PKG * LivoxHapQuickStart.PKGS_PER_MILLISEC * 100; // 默认100ms窗口
#endif
